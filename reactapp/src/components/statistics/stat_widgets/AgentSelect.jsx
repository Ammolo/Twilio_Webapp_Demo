...
const AgentSelect = ({ agents, onAgentSelect }) => {
        ...
        <div id="dropdownSearch" className="z-10 absolute bg-white rounded-lg shadow w-60 dark:bg-gray-700 mt-1">
          ...
          <ul className="max-h-48 px-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
            {filteredAgents.map(agent => (
              <li key={agent.id} className="flex items-center p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                <input
                  type="checkbox"
                  className="w-4 h-4 text-blue-600 mr-2"
                  checked={selectedAgent && selectedAgent.id === agent.id}
                  onChange={() => handleAgentSelect(agent)}
                />
                <label className="w-full text-sm font-medium text-gray-900 dark:text-gray-300" onClick={() => handleAgentSelect(agent)}>{agent.name}</label>
              </li>
            ))}
          </ul>
        </div>
