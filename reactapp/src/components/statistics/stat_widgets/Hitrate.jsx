...
const Hitrate = ({ hitrate_data, period_selection, single_date }) => {
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      series: [
        {
          name: "Hitrate",
          data: hitrate_data[0].hitrates,
          color: "#1A56DB",
        },
      ],
      xaxis: {
        categories: hitrate_data[0].period,
        labels: {
          show: false,
        },
        axisBorder: {
  ...
  return (
    ...
    >
      <div className="flex justify-between">
        <div>
          <h5 className="leading-none text-3xl font-bold text-gray-900 dark:text-white pb-2">
            {(hitrate_data[0].avg_hitrate_period * 100).toFixed(1)}%
          </h5>
          <p className="text-base font-normal text-gray-500 dark:text-gray-400">
            Hitrate {period_selection ? period_selection.toLowerCase() : ''}
          </p>
        </div>
      ...
      {!single_date && <div id="hitrate-chart" ref={chartRef}></div>}
    </div>
  );
};
